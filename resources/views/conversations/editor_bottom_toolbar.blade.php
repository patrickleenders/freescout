...
<div id="editor_bottom_toolbar" style="display:none">
	...
	<select name="status" class="form-control parsley-exclude" data-reply-status="@if ($mailbox->ticket_status == App\Mailbox::TICKET_STATUS_KEEP_CURRENT){{ $conversation->status }}@else{{ $mailbox->ticket_status }}@endif" data-note-status="{{ $conversation->status }}">
        ...
        <option value="{{ App\Mailbox::TICKET_AWAITING_CUSTOMER }}" @if ($mailbox->ticket_status == App\Mailbox::TICKET_AWAITING_CUSTOMER || ($mailbox->ticket_status == App\Mailbox::TICKET_STATUS_KEEP_CURRENT && $conversation->status == App\Mailbox::TICKET_AWAITING_CUSTOMER))selected="selected"@endif>{{ __('Wachten klant') }}</option>
        <option value="{{ App\Mailbox::TICKET_AWAITING_SUPPLIER }}" @if ($mailbox->ticket_status == App\Mailbox::TICKET_AWAITING_SUPPLIER || ($mailbox->ticket_status == App\Mailbox::TICKET_STATUS_KEEP_CURRENT && $conversation->status == App\Mailbox::TICKET_AWAITING_SUPPLIER))selected="selected"@endif>{{ __('Wachten leverancier') }}</option>
        <option value="{{ App\Mailbox::TICKET_AWAITING_TODO }}" @if ($mailbox->ticket_status == App\Mailbox::TICKET_AWAITING_TODO || ($mailbox->ticket_status == App\Mailbox::TICKET_STATUS_KEEP_CURRENT && $conversation->status == App\Mailbox::TICKET_AWAITING_TODO))selected="selected"@endif>{{ __('Todo') }}</option>
        <option value="{{ App\Mailbox::TICKET_AWAITING_PROJECT }}" @if ($mailbox->ticket_status == App\Mailbox::TICKET_AWAITING_PROJECT || ($mailbox->ticket_status == App\Mailbox::TICKET_STATUS_KEEP_CURRENT && $conversation->status == App\Mailbox::TICKET_AWAITING_PROJECT))selected="selected"@endif>{{ __('Project') }}</option>
        <option value="{{ App\Mailbox::TICKET_AWAITING_DUPLICATE }}" @if ($mailbox->ticket_status == App\Mailbox::TICKET_AWAITING_DUPLICATE || ($mailbox->ticket_status == App\Mailbox::TICKET_STATUS_KEEP_CURRENT && $conversation->status == App\Mailbox::TICKET_AWAITING_DUPLICATE))selected="selected"@endif>{{ __('Duplicaat') }}</option>
